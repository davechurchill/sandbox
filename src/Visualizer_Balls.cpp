...
void Visualizer_Balls::resetBalls()
{
    m_balls.clear();
    m_trails.clear();
...
sf::Color Visualizer_Balls::randomBallColor()
{
...
bool Visualizer_Balls::isValidTerrainPosition(
...
bool Visualizer_Balls::addBall(const cv::Point2f & position)
{
...
bool Visualizer_Balls::spawnBallNearMiddle(const cv::Mat & terrain)
{
    if (terrain.empty())
    {
        return false;
...
bool Visualizer_Balls::findRandomTerrainPosition(
    const cv::Mat & terrain,
    cv::Point2f & position)
{
    if (terrain.empty())
...
bool Visualizer_Balls::spawnRandomBall(const cv::Mat & terrain)
{
    cv::Point2f position;
    return findRandomTerrainPosition(terrain, position) && addBall(position);
}

float Visualizer_Balls::getTerrainBallRadius(const cv::Mat & terrain) const
{
    const float sizeScale = m_ballSize / 18.0f;
    return std::max(1.0f, std::min(terrain.cols, terrain.rows) * 0.012f * sizeScale);
}

void Visualizer_Balls::resolveBallCollisions(const cv::Mat & terrain)
{
...
void Visualizer_Balls::updateBalls(const cv::Mat & terrain, float deltaTime)
{
...
void Visualizer_Balls::drawBall(
...
void Visualizer_Balls::renderBallTrails(sf::RenderWindow & window)
{
    if (m_trails.empty())
    {
        return;
...
void Visualizer_Balls::renderBalls(sf::RenderWindow & window)
{
...
bool Visualizer_Balls::mapMouseToTerrain(
...
void Visualizer_Balls::handleInput(const sf::Event & event, const sf::Vector2f & mouse)
{
...
void Visualizer_Balls::render(
    sf::RenderWindow & window)
{
    renderBalls(window);
}

void Visualizer_Balls::processEvent(
...
void Visualizer_Balls::save(Settings & save) const
{
    Settings::json & settings = save.section("Visualizer_Balls");
...
void Visualizer_Balls::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_Balls");
