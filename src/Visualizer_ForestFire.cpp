...
void Visualizer_ForestFire::init()
{
    reloadShader();
}

void Visualizer_ForestFire::reloadShader()
{
...
void Visualizer_ForestFire::updateBurnableMask(const cv::Mat & terrain)
{
...
void Visualizer_ForestFire::initializeForest(const cv::Mat & terrain)
{
...
void Visualizer_ForestFire::simulateStep(const cv::Mat & terrain, float deltaTime)
{
...
void Visualizer_ForestFire::spawnFireParticle()
{
...
void Visualizer_ForestFire::updateFireParticles(float deltaTime)
{
...
void Visualizer_ForestFire::renderFireParticles(sf::RenderWindow & window)
{
...
void Visualizer_ForestFire::applyTreeBrush(
...
void Visualizer_ForestFire::paintTreeLine(
...
void Visualizer_ForestFire::ignite(const cv::Point2f & position, float radius)
{
    if (m_fuel.empty() || m_fire.empty())
    {
        return;
...
bool Visualizer_ForestFire::igniteRandomFire()
{
    if (m_fuel.empty())
    {
        return false;
...
void Visualizer_ForestFire::extinguish()
{
    if (!m_fire.empty())
    {
        m_fire.setTo(0.0f);
...
void Visualizer_ForestFire::updateStatistics()
{
    m_burningCells = 0;
    m_treeCells = 0;
    m_burningPositions.clear();
...
void Visualizer_ForestFire::updateTexture(const cv::Mat & terrain)
{
...
bool Visualizer_ForestFire::mapMouseToTerrain(
...
void Visualizer_ForestFire::imgui()
{
    PROFILE_FUNCTION();
...
void Visualizer_ForestFire::render(sf::RenderWindow & window)
{
    PROFILE_FUNCTION();
    if (!m_hasFrame)
    {
...
void Visualizer_ForestFire::processEvent(
...
void Visualizer_ForestFire::save(Settings & save) const
{
    Settings::json & settings = save.section("Visualizer_ForestFire");
...
void Visualizer_ForestFire::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_ForestFire");
...
void Visualizer_ForestFire::onSourceChanged()
{
    m_resetRequested = true;
    m_simulationAccumulator = 0.0f;
    m_particleSpawnAccumulator = 0.0f;
...
void Visualizer_ForestFire::process(const TerrainFrame & data)
{
    PROFILE_FUNCTION();
    ...
    const bool sizeChanged = m_topographySize != data.heightMap.size();
