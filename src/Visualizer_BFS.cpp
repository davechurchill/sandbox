...
void Visualizer_BFS::resetParticles()
{
    m_particles.clear();
    m_spawnAccumulator = 0.0;
}

void Visualizer_BFS::updateParticles(const cv::Mat & terrain, float deltaTime)
{
...
void Visualizer_BFS::updateTexture(
...
void Visualizer_BFS::process(
    const TerrainFrame & data)
{
    PROFILE_FUNCTION();
    updateParticles(data.heightMap, data.deltaTime);
    if (!data.heightMap.empty())
    {
        updateTexture(data.heightMap);
    }
}

void Visualizer_BFS::render(
...
{
    Settings::json & settings = save.section("Visualizer_BFS");
...
void Visualizer_BFS::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_BFS");
    Settings::read(settings, "m_trailLength", m_trailLength);
    Settings::read(settings, "m_cellSize", m_cellSize);
    Settings::read(settings, "m_spawnRate", m_spawnRate);
    Settings::read(settings, "m_particleSpeed", m_particleSpeed);
    Settings::read(settings, "m_particleAlpha", m_particleAlpha);
    Settings::read(settings, "m_heightPenalty", m_heightPenalty);
    m_resetRequested = true;
}
