...
bool Overlay_AStar::isTraversable(int x, int y) const
{
...
bool Overlay_AStar::mapMouseToTerrain(
...
void Overlay_AStar::precalculateSlopes()
{
...
void Overlay_AStar::calculatePath()
{
    m_path.clear();
    m_pathDistance = 0.0f;
    m_pathCost = 0.0f;
    ...
    TopographyProcessor & processor) const
{
    ...
    const cv::Mat projection = processor.projector().getProjectionMatrix();
    ...
    const sf::Vector2f origin = processor.projector().getTransformedPosition();
...
void Overlay_AStar::clearPoints()
{
    m_path.clear();
    m_pathDistance = 0.0f;
    m_pathCost = 0.0f;
...
void Overlay_AStar::initOverlay()
{
    clearPoints();
}

void Overlay_AStar::imguiOverlay()
{
    PROFILE_FUNCTION();
...
void Overlay_AStar::processTopographyOverlay(
        ...
        && m_topographySize != data.topography.size())
    {
        ...
        if (m_hasStart)
        {
            m_start.x = std::clamp(
                ...
                0,
                data.topography.cols - 1);
            m_start.y = std::clamp(
                (int)std::round(m_start.y * yScale),
                0,
                data.topography.rows - 1);
        }
        if (m_hasGoal)
        {
            m_goal.x = std::clamp(
...
void Overlay_AStar::renderOverlay(
    sf::RenderWindow & window,
    TopographyProcessor & processor)
{
    PROFILE_FUNCTION();
    drawPath(window, processor);
}

void Overlay_AStar::processOverlayEvent(
    ...
    TopographyProcessor & processor)
{
    ...
    cv::Point point;
    if (!mapMouseToTerrain(mouse, processor, point))
    {
        return;
    }
...
void Overlay_AStar::saveOverlay(Settings & save) const
{
    Settings::json & settings = save.section("Overlay_AStar");
...
void Overlay_AStar::loadOverlay(const Settings & save)
{
    const Settings::json & settings = save.section("Overlay_AStar");
