...
void Visualizer_Heat::setInitialHeatSources()
{
...
void Visualizer_Heat::processEvent(const sf::Event& event, const sf::Vector2f& mouse)
{
    PROFILE_FUNCTION();
...
void Visualizer_Heat::process(const TerrainFrame& data)
{
    PROFILE_FUNCTION();

    {
...

        {
            ...
            projector().project(data.heightMap, m_cvTransformedDepthImage32fColor);
        }
    ...
    {
        PROFILE_SCOPE("Heat");
        m_heatGrid.update(data.heightMap, m_iterations);

        if (m_doStep)
        {
            m_heatGrid.update(data.heightMap, 1);
            m_doStep = false;
        }
