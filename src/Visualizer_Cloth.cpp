...
float Visualizer_Cloth::vectorLength(const Vector3 & vector)
{
    return std::sqrt(vector.x * vector.x + vector.y * vector.y + vector.z * vector.z);
}

int Visualizer_Cloth::nodeIndex(int column, int row) const
{
    return row * (m_cellsX + 1) + column;
}

float Visualizer_Cloth::sampleHeight(const cv::Point2f & position) const
{
...
float Visualizer_Cloth::heightScale() const
{
    return std::max(1.0f, std::min(m_topography.cols, m_topography.rows) * 0.35f);
}

void Visualizer_Cloth::addSpring(int first, int second, float stiffnessScale)
{
    const Vector3 delta{
...
void Visualizer_Cloth::createCloth()
{
...
void Visualizer_Cloth::updateCloth(float deltaTime)
{
    if (m_nodes.empty())
    {
        return;
...
bool Visualizer_Cloth::mapMouseToTerrain(
...
void Visualizer_Cloth::renderCloth(
    ...
    const TerrainFrame & data)
{
    ...
    {
        m_center.x *= (float)data.heightMap.cols / m_topographySize.width;
        m_center.y *= (float)data.heightMap.rows / m_topographySize.height;
        m_resetPending = true;
    }
...
void Visualizer_Cloth::render(
    sf::RenderWindow & window)
{
    renderCloth(window);
}

void Visualizer_Cloth::processEvent(
...
void Visualizer_Cloth::save(Settings & save) const
{
    Settings::json & settings = save.section("Visualizer_Cloth");
...
void Visualizer_Cloth::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_Cloth");
