...
void Visualizer_Weather::resetParticles()
{
    m_particles.clear();
}

float Visualizer_Weather::sampleHeight(
...
void Visualizer_Weather::initializeParticle(
    Particle & particle,
    const cv::Mat & terrain,
    bool randomAltitude)
{
...
void Visualizer_Weather::wrapPosition(cv::Point2f & position, const cv::Size & size) const
{
...
int Visualizer_Weather::targetParticleCount() const
{
    int maximum = 0;
    switch ((Mode)m_mode)
    {
...
void Visualizer_Weather::updateParticles(const cv::Mat & terrain, float deltaTime)
{
...
void Visualizer_Weather::renderPrecipitation(sf::RenderWindow & window, bool snow)
{
...
void Visualizer_Weather::renderAtmosphere(sf::RenderWindow & window, bool clouds)
{
    if (m_particles.empty())
    {
        return;
...
void Visualizer_Weather::init()
{
    resetParticles();
}

void Visualizer_Weather::imgui()
{
    PROFILE_FUNCTION();
...
void Visualizer_Weather::process(
...
void Visualizer_Weather::render(
...
void Visualizer_Weather::processEvent(
    const sf::Event &,
    const sf::Vector2f &)
{
}

void Visualizer_Weather::save(Settings & save) const
{
    Settings::json & settings = save.section("Visualizer_Weather");
...
void Visualizer_Weather::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_Weather");
    Settings::read(settings, "m_mode", m_mode);
    Settings::read(settings, "m_intensity", m_intensity);
    Settings::read(settings, "m_windX", m_windX);
    Settings::read(settings, "m_windY", m_windY);
    Settings::read(settings, "m_elementSize", m_elementSize);
    Settings::read(settings, "m_fallSpeed", m_fallSpeed);
    resetParticles();
}
