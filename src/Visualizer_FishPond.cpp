...
void Visualizer_FishPond::init()
{
    reloadShader();
    resetFish();
}

void Visualizer_FishPond::reloadShader()
{
...
float Visualizer_FishPond::sampleHeight(const cv::Point2f & position) const
{
...
bool Visualizer_FishPond::isSwimmable(const cv::Point2f & position) const
{
    return isSwimmable(position, m_minimumDepth);
}

bool Visualizer_FishPond::isSwimmable(
...
void Visualizer_FishPond::randomizeWander(Fish & fish)
{
...
bool Visualizer_FishPond::addFish(const cv::Point2f & position)
{
...
bool Visualizer_FishPond::spawnRandomFish()
{
...
void Visualizer_FishPond::resetFish()
{
    m_fish.clear();
    m_resetPending = true;
}

cv::Point2f Visualizer_FishPond::shorelineSteering(
    const Fish & fish,
...
void Visualizer_FishPond::updateFish(float deltaTime)
{
    if (m_topography.empty())
    {
        return;
...
bool Visualizer_FishPond::mapMouseToTerrain(
...
void Visualizer_FishPond::drawFish(
...
void Visualizer_FishPond::renderFish(sf::RenderWindow & window)
{
    if (m_fish.empty())
    {
        return;
...
void Visualizer_FishPond::imgui()
{
    PROFILE_FUNCTION();
...
void Visualizer_FishPond::render(sf::RenderWindow & window)
{
    PROFILE_FUNCTION();
...
void Visualizer_FishPond::processEvent(
...
void Visualizer_FishPond::save(Settings & save) const
{
    Settings::json & settings = save.section("Visualizer_FishPond");
...
void Visualizer_FishPond::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_FishPond");
    Settings::read(settings, "m_targetFishCount", m_targetFishCount);
    Settings::read(settings, "m_speedMultiplier", m_speedMultiplier);
    Settings::read(settings, "m_fishSize", m_fishSize);
    Settings::read(settings, "m_schoolRadius", m_schoolRadius);
    Settings::read(settings, "m_schoolStrength", m_schoolStrength);
    Settings::read(settings, "m_separationStrength", m_separationStrength);
    Settings::read(settings, "m_minimumDepth", m_minimumDepth);
    Settings::read(settings, "m_maximumFishDepth", m_maximumFishDepth);
    m_resetPending = true;
}

void Visualizer_FishPond::process(const TerrainFrame & data)
{
    PROFILE_FUNCTION();
