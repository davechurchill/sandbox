...
void Visualizer_WaterFlow::ensureSimulationSize(const cv::Size & size)
{
...
void Visualizer_WaterFlow::resetWater()
{
...
void Visualizer_WaterFlow::addRain(const cv::Mat & terrain, float amount)
{
    if (m_rainMode == 0)
    {
...
void Visualizer_WaterFlow::simulate(const cv::Mat & terrain, float deltaTime)
{
...
void Visualizer_WaterFlow::buildImage(const cv::Mat & terrain)
{
    cv::Mat terrain8u;
...
void Visualizer_WaterFlow::imguiControls()
{
    PROFILE_FUNCTION();
...
void Visualizer_WaterFlow::renderWater(sf::RenderWindow & window)
{
    if (m_hasFrame)
    {
...
void Visualizer_WaterFlow::handleInput(const sf::Event & event, const sf::Vector2f & mouse)
{
...
bool Visualizer_WaterFlow::mapMouseToTerrain(const sf::Vector2f & mouse, cv::Point2f & terrainPosition)
{
    if (m_water.empty())
    {
        return false;
...
void Visualizer_WaterFlow::init()
{
    resetWater();
    reloadShader();
}

void Visualizer_WaterFlow::imgui()
{
    imguiControls();
}

void Visualizer_WaterFlow::process(
    const TerrainFrame & data)
{
    ...
    simulate(data.heightMap, data.deltaTime);
    buildImage(data.heightMap);
}

void Visualizer_WaterFlow::render(
    sf::RenderWindow & window)
{
    renderWater(window);
}

void Visualizer_WaterFlow::processEvent(
...
void Visualizer_WaterFlow::save(Settings & save) const
{
    Settings::json & settings = save.section("Visualizer_WaterFlow");
...
void Visualizer_WaterFlow::load(const Settings & save)
{
    const Settings::json & settings = save.section("Visualizer_WaterFlow");
